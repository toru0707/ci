...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.omnibus.chef_version = :latest
	config.vm.define :jenkins do |jenkins|
		...
		jenkins.vm.network :forwarded_port, guest: 8080, host: 28080, auto_correct: true
  ...
  config.vm.define :gitlab do |gitlab|
		gitlab.vm.network :private_network, ip: "192.168.1.3"
		gitlab.vm.network :forwarded_port, guest: 80, host: 30080, auto_correct: true
    gitlab.vm.provider :virtualbox do |vb|
      vb.customize ["modifyvm", :id, "--memory", 2048]
    end
		gitlab.vm.provision :chef_solo do |chef|
			chef.run_list = ["chef-timezone","mysql::server", "gitlab"]
			chef.json = {
        timezone:  {
          zone: "Asia/Tokyo"
        },
